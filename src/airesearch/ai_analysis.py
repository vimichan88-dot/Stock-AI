...
def build_input(report: Report, news_items: list[NewsItem]) -> str:
    ...
    required_schema = {
        ...
        "core_events": [
            {
                "title": "string",
                "summary": "string，网页会显示为“最新动态”，只能写提炼后的可复核事实、量化线索和投研解读；严禁写消息来源、发布时间、检索主题、原始标题、链接",
                "reason": "string，网页会显示为“市场影响”，必须基于该事件内容详细解释利好/利空方向、传导链条、A股/港股/美股映射、量化观察指标和投资动作建议",
                ...
                "sources": ["string，专门放消息来源、发布时间、检索主题、原始标题或链接"],
            }
        ],
        "investment_ideas": [
            {
    ...
    return json.dumps(
        {
            ...
            "quality_rules": [
                ...
                "core_events[].summary会显示在网页的“最新动态”，只能写提炼后的事实、量化线索和投研解读；严禁写消息来源、发布时间、检索主题、原始标题、链接，这些只能放到sources。",
                "最新动态必须具体，不能写泛泛提示；如果标题没有量化信息，就说明可复核事实是什么，例如具体政策、公司动作、指数变化、订单/价格/资金规模。",
                "市场影响必须基于该事件内容展开，解释利好还是利空、为什么、传导链条是什么、A股/港股/美股如何映射，并给出量化观察指标和投资动作建议。",
                "每条核心事件必须分别给出利好和利空股票清单；不能写待复核。",
                "analysis_sections必须引用evidence_packet中的近期具体事件和量化线索，不能写成通用研究框架或长期模板。",
                "analysis_sections至少覆盖：十大事件逻辑、全球市场复盘、产业趋势、资金流向、政策公告、未来7天、投资机会行动清单。",
                "投资机会必须给出催化剂、观察指标、定价状态、仓位和失效条件。",
                "如果证据不足，明确写等待确认，不能硬编数字。",
                "只输出合法JSON，不要Markdown，不要解释文本；字符串里的换行也要正确转义。",
            ],
        },
        ensure_ascii=False,
    )
